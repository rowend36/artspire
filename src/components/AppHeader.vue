...
<template>
  <nav v-if="isWideScreen" class="app-padding nav-bar">
    ...
    <RouterLink v-for="url of links" :key="url.href" :to="url.href" :class="['header-link', { 'header-download-link': !!url.icon }]">{{ url.name }}
  ...
  <nav v-else class="app-padding nav-bar">
    <app-button variant='icon' id='menu-btn'>
      <MenuIcon />
    </app-button>
    <RouterLink v-for="url of links.filter(e=>e.showOnMobile)" :key="url.href" :to="url.href" :class="['header-link', { 'header-download-link': !!url.icon }]">{{ url.name }}
      <svg v-if="url.icon" width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path :d="url.icon" fill="#162574" />
      </svg>
    </RouterLink>
